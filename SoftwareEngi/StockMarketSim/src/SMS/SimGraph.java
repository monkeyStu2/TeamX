...
public class SimGraph extends JPanel {
    ...
    public void createDataset(LocalDate date, LocalTime time, ArrayList<Company> companies) {
        dataset = new TimeSeriesCollection();
        LocalDateTime ldt = LocalDateTime.of(date, time);
        Date dateTime = Date.from(ldt.atZone(ZoneId.systemDefault()).toInstant());
        for (Company company : companies) {
            TimeSeries data = new TimeSeries(company.getName());
            data.add(new Minute(dateTime), company.getSharePrice());
    ...
    public void updateDataset(LocalDate date, LocalTime time, ArrayList<Company> companies) {
        for (int i = 0; i < dataset.getSeriesCount(); i++) {
            ...
            LocalDateTime ldt = LocalDateTime.of(date, time);
            Date dateTime = Date.from(ldt.atZone(ZoneId.systemDefault()).toInstant());
            ser.add(new Minute(dateTime), companies.get(i).getSharePrice());
        }
        setDataset(dataset);
    }
